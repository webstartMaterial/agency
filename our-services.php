...
<div class="container px-5 py-24 mx-auto flex flex-col">
  ...
  <div class="flex">
    <nav class="flex text-sm <?= $textParagraphLightMode; ?> dark:text-gray-300 space-x-2 items-center" aria-label="Breadcrumb">
      <a href="/" class="inline-flex items-center hover:<?= $textPrimaryHover; ?> transition">
...
<section class="<?= $textParagraphLightMode; ?> dark:text-gray-300 body-font overflow-hidden bg-white dark:<?= $bgSecondary; ?>">
    ...
    <div class="lg:w-6/6 mx-auto">
      ...
      <div class="flex flex-col sm:flex-row mt-10">
        <div class="sm:w-1/3 text-center sm:pr-8 sm:py-8">
          <div
            class="w-20 h-20 rounded-full inline-flex items-center justify-center bg-gray-200 <?= $textParagraph; ?> dark:<?= $bgQuartenary; ?> dark:text-gray-300">
          ...
          <div class="flex flex-col items-center text-center justify-center">
            <h2 class="font-medium title-font mt-4 <?= $textTitleLightMode;?> <?= $textTitle;?> text-lg">
              <?= htmlspecialchars($content['presentation']['intro']['name']) ?>
            </h2>
            <div class="w-12 h-1 <?= $bgPrimary;?> rounded mt-2 mb-4"></div>
        ...
        <div
          ...
          <div class="mt-8">
            <a href="#packs"
              class="inline-flex items-center px-6 py-3 <?= $bgPrimary;?> hover:<?= $bgPrimaryHover; ?> dark:<?= $textTitle; ?> text-lg font-semibold rounded-2xl shadow-lg transition duration-300 ease-in-out">
    ...
    <div class="flex flex-wrap -m-4">
      <?php foreach ($content['pricing']['packs'] as $index => $pack): ?>
        <div class="p-4 xl:w-1/4 md:w-1/2 w-full" data-aos="flip-up" data-aos-delay="<?= $index * 150 ?>">
          <div
            class="h-full p-6 rounded-lg border-2 <?= $pack['highlight'] ? $border : 'border-gray-300 dark:border-gray-700' ?> bg-white dark:<?= $bgSecondary ;?> flex flex-col relative overflow-hidden transition-all duration-300">
            ...
            <h1
              class="text-5xl <?= $textTitleLightMode;?> <?= $textTitle;?> leading-none flex items-center pb-4 mb-4 border-b border-gray-200 dark:border-gray-700">
              <span><?= htmlspecialchars($pack['price']) ?></span>
            </h1>

            <?php foreach ($pack['features'] as $feature): ?>
              <p class="flex items-center <?= $textParagraphLightMode; ?> dark:text-gray-300 mb-2">
                <span
                  class="w-4 h-4 mr-2 inline-flex items-center justify-center bg-gray-400 dark:<?= $bgPrimary;?> dark:<?= $textTitle; ?> rounded-full flex-shrink-0">
            ...
            <div class="mt-auto">
              <button
                class="mt-4 flex items-center w-full dark:<?= $textTitle; ?> <?= $pack['highlight'] ? "$bgPrimary hover:$bgPrimary" : 'bg-gray-400 dark:<?= $bgQuartenary; ?> hover:<?= $bgQuartenaryHover; ?> dark:hover:<?= $bgQuartenaryHover; ?>' ?> border-0 py-2 px-4 focus:outline-none rounded transition">
            ...
            <?php if (!empty($pack['options'])): ?>
              <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                <p class="text-xs <?= $textParagraphLightMode; ?> dark:<?= $textParagraph; ?> font-semibold mb-2 uppercase tracking-wide">Options
                  disponibles</p>
                <?php foreach ($pack['options'] as $option): ?>
                  <p class="text-sm <?= $textParagraphLightMode; ?> dark:text-gray-300 mb-1">• <?= htmlspecialchars($option) ?></p>
                <?php endforeach; ?>
              </div>
            <?php endif; ?>
